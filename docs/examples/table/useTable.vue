<script setup lang="ts">
  import { defineTableColumns, useEpTable, EpTable } from 'e-plus-ui'
  import { ref } from 'vue'
  const { registry, addFieldColumns, deleteFieldColumns, updateFieldColumns, clearSelection, toggleRowSelection } = useEpTable<typeof tableData[0]>()
  const index = ref(0)
  const tableData = [
    {
      id: 1,
...
<template>
  <div>
    <div style="padding: 5px">
      ...
      <el-button type="primary" @click="handleDynamicNameTo('name', true)">添加到【名称】前</el-button>
      <el-button type="primary" @click="handleDynamicNameTo('name')">添加到【名称】后</el-button>
      <el-button type="primary" @click="handleDynamicNameTo('date', true)">添加到【日期】前</el-button>
      ...
      <el-divider></el-divider>
      <el-button type="warning" @click="selectionBatch1">选中【重庆，湖南】所在行（通过数据行）</el-button>
      <el-button type="warning" @click="selectionBatch2">选中【重庆，湖南】所在行（通过唯一键）</el-button>
    </div>
    <el-divider></el-divider>
    <EpTable row-key="id" @registry="registry" :data="tableData" :column>
    </EpTable>
  </div>
