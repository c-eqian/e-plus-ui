...
export const useColumns = (getTableInstance: () => any) => {
  const filterType = (typeValue?: string) => {
    return (
      typeValue &&
      ['index', 'selection', 'expand', 'operation'].includes(typeValue)
    );
  };
  /**
   * 新增列数
   ...
   * @param args
   */
  const addFieldColumns = (
    columns: ITableColumnConfig[] | ITableColumnConfig,
    ...[to, front]
  ) => {
    const instance = getTableInstance();
    const columnsData = instance?.getColumns() as Ref<ITableColumnConfig[]>;
    ...
    const insertTo = (_front: boolean, index?: number) => {
      const newColumns: ITableColumnConfig[] = !isArray(columns)
        ? [columns]
        : columns;
      if (index === undefined || index > columnsData.value.length) {
        if (_front) {
          ...
          const isFilterType =
            filterType(lastColumn?.type) ||
            !isEmpty(lastColumn?.operationType ?? '');
          isFilterType
  ...
  const deleteFieldColumns = (
    propValues: ITableColumnConfig['prop'][] | ITableColumnConfig['prop']
  ) => {
    const instance = getTableInstance();
    const columnsData = instance?.getColumns() as Ref<ITableColumnConfig[]>;
    if (isArray(propValues)) {
      ...
      const index = columnsData.value.findIndex(
        (column) => column.prop === propValues
      );
      if (index > -1) {
        columnsData.value.splice(index, 1);
        return instance?.updateColumns(columnsData.value);
  ...
  const updateFieldColumns = (
    ...
    if (isArray(columns)) {
        ...
        for (const entry of entries) {
          ...
          if (originColumn.prop === column.prop) {
            if (update) {
              ...
              columnsData.value[+index] = useMerge(
      ...
      const index = columnsData.value.findIndex(
        (column) => column.prop === columns.prop
      );
      if (index > -1) {
        if (update) {
