...
import type { FormItemRules, FormItemsSchema, FormSchemaType } from '../type';
 ...
 * @param isSearch
 */
export const useFormItemProps = (props: FormItemsSchema, model: any, isSearch?: boolean) => {
 ...
 * @param getModel
 */
export const useFormProps = (props: ComputedRef<FormItemsSchema>, getModel: () => Ref<any>) => {
...
export const useFormItem = (
  ...
  const appendFields = (items: FormItemsSchema | FormItemsSchema[], to?: string | boolean) => {
    const formSchemas = getFormSchema();
   ...
   * @param to
   */
  const updateOrAppendFields = (prop: string, item: FormItemsSchema, to?: string) => {
    if (!prop) return;
    const formSchemas = getFormSchema();
    const _indexProp = formSchemas.value.findIndex(schema => schema.prop === prop);
    if (_indexProp > -1) {
