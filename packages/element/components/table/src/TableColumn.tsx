...
import type { TableColumnConfig, TagType, TRender } from './type';
...
export default defineComponent({
  ...
  render() {
     ...
     * @param $index
     */
    const renderOperation = (item: TableColumnConfig, row: any, $index: number) => {
     ...
     * @param item
     */
    const getSlots = (item: TableColumnConfig) => {
      // 检测是否开启表单校验
      const hasRules = useFormValidation && !(item.children?.length || !item.prop || !item?.rules);
      /**
       * 表单渲染
       * @param index
       * @param render
       * @param row
       */
      const formItemRender = (index: number, render: () => any, row: any = null) => {
        if (item.isCopy) {
          return (
            <EpCopyText>
              <ElButton link type="primary" onClick={() => this.eventListeners.view?.(row, index)}>
                {render()}
              </ElButton>
            </EpCopyText>
          );
        ...
        return (
          <ElFormItem {...formItemProp} key={item.prop} class="column-form-item__mini">
            {render()}
          </ElFormItem>
        );
      };
      ...
      const filterPropValidator = (index: number) => {
        ...
        if (this.$props.useFormValidation) {
          const propsHasRules = columns.filter(columnItem => columnItem.rules);
          return propsHasRules
            ? propsHasRules.map(propRulesItem => `data[${index}].${propRulesItem.prop}`)
            : [];
        }
        return [];
      };
       ...
       * @param prop
       */
      const deepObjectValue = (row: TableColumnConfig, prop: string): any => {
        if (!prop) return prop;
      ...
      return {
         ...
         */
        header: (() => {
          if (this.$slots.header || (item.headerRender && isFunction(item.headerRender))) {
        ...
        default:
          !item.type || ['operation', 'expand'].includes(item.type) || item.dictKey
            ? ({ row, $index }: any) => {
                if ($index < 0 && !item.render) return deepObjectValue(row, item.prop!);
                  ...
                  () => {
                    const renderCtx = () => {
                      ...
                      if (item.fixed || item.type === 'operation') {
                        ...
                        if (!item.slotKey) return renderOperation(item, row, $index);
                      }
                      if (this.$slots[item.slotKey as string] || this.$slots[item.prop as string]) {
                        /**
                         * 默认先使用自定义的插槽名称
                         * slotKey相当于prop的值别名
                         * 如果没有默认值，使用prop的值
                         */
                        return (
                          this.$slots[item.slotKey as string] || this.$slots[item.prop as string]
                        )?.({
                          row,
                          $index,
                          index: $index,
                      ...
                      if (item.dictEnum) {
                        ...
                        if (tagType) {
                          return (
                            <ElTag type={tagType}>
                              {useLabelByVal(item.dictEnum, deepObjectValue(row, item.prop!))}
                            </ElTag>
                          );
                        }
                        return useLabelByVal(item.dictEnum, deepObjectValue(row, item.prop!));
                      }
                      /**
                       * 格式化数据
                       */
     ...
     * @param item
     */
    const columnRender = (item: TableColumnConfig) => {
      ...
      return <ElTableColumn {..._columnProps} v-slots={getSlots(item)}></ElTableColumn>;
    };
