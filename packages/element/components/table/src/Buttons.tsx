import { isArray, isFunction, isObject, isString } from '@eqian/utils-vue';
    ...
    const handleClickEvents = (_type: string) => {
      if (['add', 'edit', 'view', 'delete'].includes(_type)) {
        if (eventListeners[_type] && isFunction(eventListeners[_type])) {
          return eventListeners[_type](propsComputed.value.row, propsComputed.value.index);
        }
      }
      if (eventListeners.btn && isFunction(eventListeners.btn)) {
        return eventListeners.btn(_type, propsComputed.value.row, propsComputed.value.index);
      }
    };
    const createBaseOperation = (item: OperationType, label?: string, extra = {}) => {
      ...
      if (isArray(operationType)) {
        return operationType.map(item => {
          if (isString(item)) {
            ...
            const { type = '', permission, label = '', extra = {} as any, render } = item;
            const isRender = () => {
            ...
            if (['add', 'edit', 'view', 'delete'].includes(type)) {
              return isRender() ? createBaseOperation(type as OperationType, label, extra) : null;
            } else {
              return isRender()
